...
void writeHistogram(histogramT * histP, char * headersDir)
{
    ...
    //chop off the / if there is one
    int len = strlen(headersDir);
    char lastchar = headersDir[len - 1];
    if (lastchar == '/') headersDir[len - 1] = 0;
...
void buildAllHeaders(char * imagesDir, char * headersDir)
{
    ...
    if (d)
    {
        while ((dir = readdir(d)) != NULL)
        {
            ...
            {
		int len = strlen(imagesDir);
		if (imagesDir[len - 1] == '/') imagesDir[len - 1] = 0;
